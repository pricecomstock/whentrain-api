// https://github.com/seamapi/prefixed-api-key/blob/main/src/index.ts
...
export const extractLongToken = (token: string) => token.split('_').slice(-1)?.[0];

export const extractShortToken = (token: string) => token.split('_')?.[1];

export const extractLongTokenHash = (token: string) => hashLongToken(extractLongToken(token));
...
export const checkAPIKey = (token: string, expectedLongTokenHash: string): boolean => {
