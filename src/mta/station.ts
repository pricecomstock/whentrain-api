...
export async function fetchAndParseStationCSV(): Promise<stationCSVRecord[]> {
	...
	try {
		stationCsv = await fetchStationCSVFromMTA();
	} catch (error) {
		stationCsv = fetchStationCSVLocally();
	}
...
async function fetchStationCSVFromMTA(): Promise<string> {
	const response = await fetch(MTA_STATION_CSV_URL);
	logger.info("Fetched station CSV from MTA's website");
	return response.text();
}

function fetchStationCSVLocally(): string {
	logger.info('Loading local backup station CSV');
...
export const stationsList = await fetchStations();
...
export const stationsByTrain: Map<string, Station[]> = stationsList.reduce((accMap, station) => {
